...
def calculateFitNumber(student, school):
	...
	if (student["Preferred Degree"] != "NO_PREFERENCE"):
		# If the student's preferred degree is greater than the school's highest awarded degree
		if (int(student["Preferred Degree"]) > int(school["HIGHDEG"])):
			return -100
	...
	for pref_major in student["Preferred Major"]:
	...
	for pref_state in student["Preferred State"]:
	...
	if (school["GRAD_DEBT_MDN"] < (STUDENT_DEBT_MAX+STUDENT_DEBT_MIN)/2.0):
		cost_value += 1
	...
	# Check tuition
	if (student["tuition_limit"] < school["COSTT4A"]):
		cost_value += 1
